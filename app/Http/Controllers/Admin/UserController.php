    ...
    public function index(Request $request){
        if(\request()->ajax()){
            if($request->filled('chart_query')){
                ...
                // ->whereMonth('created_at', '>=', intval(substr($request->start,-7,2)))->whereMonth('created_at', '<=', intval(substr($request->end,-7,2)))
                // ->whereYear('created_at','>=',intval(substr($request->start,-4,4)))->whereYear('created_at','<=',intval(substr($request->end,-4,4)))
                // ->whereDay('created_at','>=',intval(substr($request->start,-10,2)))->whereDay('created_at','<=',intval(substr($request->end,-10,2)))
