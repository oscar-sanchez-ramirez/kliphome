...
class ApiServiceController extends ApiController
{
    ...
    {
        $final_orders = [];
        $orders = DB::table('orders as o')->join('users as u','u.id','o.user_id')->join('addresses as a','o.address','a.id')->whereNotIn('o.id',$selectedOrders)->select('o.*','a.delegation','a.address','u.name','u.lastName')->get();
        Log::notice($orders);
