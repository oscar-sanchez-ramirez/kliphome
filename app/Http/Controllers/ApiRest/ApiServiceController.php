...
class ApiServiceController extends ApiController
{
    ...
    public function ordersAccepted($user_id){
        ...
        ->select('o.*','a.municipio','a.alias','a.reference','a.interior','a.colonia','a.postal_code','a.exterior','a.street as address','u.name','u.lastName','u.avatar','so.id as idOrderAccepted','so.created_at as orderAcepted')->distinct('o.id')->orderBy('o.created_at',"DESC")->get();
        foreach ($orders as $key) {
            $category = $this->table($key->type_service,$key->selected_id);
            $key->service = $category[0]->service;
            // $key->service = $category[0]->service;
