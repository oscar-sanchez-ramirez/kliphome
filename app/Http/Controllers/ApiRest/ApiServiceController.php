...
class ApiServiceController extends ApiController
{
    ...
    public function categories($ids,$postal_code,$user_id)
    {
        ...
        ->whereIn('a.postal_code',$postal_code)
            ->select('o.*','a.delegation','a.street as address','a.postal_code','u.name','u.lastName','u.avatar')->get();
    ...
    public function ordersAccepted($user_id){
        ...
        ->select('o.*','a.delegation','a.alias','a.street as address','u.name','u.lastName','u.avatar','so.id as idOrderAccepted','so.created_at as orderAcepted')->distinct('o.id')->get();
        foreach ($orders as $key) {
            $category = $this->table($key->type_service,$key->selected_id);
            $key->service = $category[0]->service;
            $key->service = $category[0]->service;
