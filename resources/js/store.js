...
export default new Vuex.Store({
    ...
    actions: {
      getMessages(context,conversation){
        axios.get('/api/messages?contact_id='+conversation.contact_id+'&user_id='+conversation.user_id+'&conversation_id='+conversation.id).then(
          response=>{
            ...
            window.Echo.removeListener('users.'+this.state.selectedConversation.id+'.'+this.state.user.id);
      ...
      openChannel(context,id){
        console.log(id)
        window.Echo.private('users.'+id+'.'+this.state.user.id).listen('MessageSent',(data)=>{
